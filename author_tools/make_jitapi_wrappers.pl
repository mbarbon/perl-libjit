...
my ($input, $output_c, $output_h, $output_xsp) = @ARGV;

unless ($input && $output_c && $output_h && $output_xsp) {
    print STDERR <<EOH;
Usage: $0 <input C file> <output C file> <output H file> <output XSP file>
EOH
    exit 1;
}
...
jit_value_t ${func}(jit_function_t function, jit_value_t thx${jitparms})
{
...
sub process_file {
    my ($input, $output_c, $output_h, $output_xsp) = @_;
    my $text;
...
process_file($input, $output_c, $output_h, $output_xsp);
